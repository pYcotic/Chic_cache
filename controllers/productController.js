...
const getOneProduct = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)){
        return res.status(404).json({error: `id:${id} is not a valid id`})
    }
...
const updateProduct = async (req, res) => {
    ...
    if (!product)  {
        return res.status(404).json({error: `Product with id:${id}, does not exist and/or was not updated`})
    }
    res.status(200).json(product)
...
const deleteProduct = async (req, res) => {
    ...
    if (!deletedProduct) {
        return res.status(404).json({error: `Product with id:${id} does not exist and therefore cannot be deleted`})
    }
